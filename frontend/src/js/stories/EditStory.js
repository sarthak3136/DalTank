...
const EditStory = () => {
  ...
  useEffect(() => {
    // Fetch the existing story data when the component mounts
    const fetchStoryData = async () => {
      try {
        const response = await axios.get(
          `https://group12-backend.onrender.com/stories/${id}`
        );
        const storyData = response.data;
        console.log();
        setFirstName(storyData.firstName);
        setLastName(storyData.lastName);
        setCompanyName(storyData.companyName);
        setCompanyEmail(storyData.companyEmail);
  ...
  const onSubmit = async (e) => {
    ...
    try {
      const response = await axios.put(
        "https://group12-backend.onrender.com/stories/user/" + id,
        storyData
      );
      ...
      <Grid container direction="column" style={{ marginTop: "5.5rem" }}>
        ...
        >
          <SnackbarContent
            ...
            message="Failed to submit the story. Please try again."
          />
        </Snackbar>
      </Grid>
    </div>
